...
namespace Zeron
{
	...
	GraphicsVulkan::~GraphicsVulkan()
	{
		mDevice.destroy();
#if ZE_DEBUG
		VulkanDebugger::Destroy(mInstance);
#endif
		mInstance.destroy();
	}
	...
	{
		DispatchLoaderVulkan::InitInstanceDispatcher();
		...
		DispatchLoaderVulkan::InitInstanceDispatcher(mInstance);

	#if ZE_DEBUG
		if (!validationLayers.empty()) {
			VulkanDebugger::Create(mInstance);
		}
	#endif
		return mInstance;
	}
