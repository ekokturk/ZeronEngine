...
namespace Zeron
{
	...
	class ComponentContainer final : public IComponentContainer {
	  public:
		...
		Type& GetComponent(Entity::Id entityId) { return mContainer.Get(entityId).first; }

		Type& GetComponent(Entity::Id entityId) const { return mContainer.Get(entityId).first; }

		Type* TryGetComponent(Entity::Id entityId) { return &mContainer.TryGet(entityId)->first; }

		bool Has(Entity::Id entityId) const override { return mContainer.Contains(entityId); }
		...
		const std::type_info& GetType() const override { return typeid(Type); }

		size_t GetSize() const { return mContainer.GetSize(); }

		std::vector<TypeIndexPair>& GetComponentList() { return mContainer.GetData(); }
