...
namespace Zeron
{
	...
	class Color {
	  public:
		...
		bool operator==(Palette color) const { return this->HexRGB() == static_cast<uint32_t>(color); }
		...
		std::array<float, 4> norm() const { return { normR(), normG(), normB(), normA() }; }
		...
		static Color FromNorm(float r, float g, float b, float a = 1.f)
		{
			return Color(static_cast<uint8_t>(r * 255), static_cast<uint8_t>(g * 255), static_cast<uint8_t>(b * 255), static_cast<uint8_t>(a * 255));
		}

		static uint32_t HexFromRGB(uint8_t r, uint8_t g, uint8_t b) { return r << 16 | g << 8 | b; }
		...
		static uint32_t HexFromARGB(uint8_t r, uint8_t g, uint8_t b, uint8_t a) { return a << 24 | HexFromRGB(r, g, b); }

		static uint32_t HexFromARGB(float r, float g, float b, float a)
		{
			return static_cast<uint8_t>(std::clamp(a, 0.f, 1.f) * 255) << 24 | HexFromRGB(std::clamp(r, 0.f, 1.f), std::clamp(g, 0.f, 1.f), std::clamp(b, 0.f, 1.f));
		}

		static uint32_t HexFromRGBA(uint8_t r, uint8_t g, uint8_t b, uint8_t a) { return HexFromRGB(r, g, b) << 8 | a; }

		static uint32_t HexFromRGBA(float r, float g, float b, float a)
		{
			return HexFromRGB(std::clamp(r, 0.f, 1.f), std::clamp(g, 0.f, 1.f), std::clamp(b, 0.f, 1.f)) << 8 | static_cast<uint8_t>(std::clamp(a, 0.f, 1.f) * 255);
		}
