...
namespace Zeron
{
	...
	class TVec2 {
	  ...
	  public:
		...
		TVec2 operator+(const TVec2& other) const { return { X + other.X, Y + other.Y }; }

		TVec2 operator-(const TVec2& other) const { return { X - other.X, Y - other.Y }; }

		TVec2 operator+(T value) const { return { X + value, Y + value }; }

		TVec2 operator-(T value) const { return { X - value, Y - value }; }

		TVec2 operator*(T value) const { return { X * value, Y * value }; }

		TVec2 operator/(T value) const { return value == 0 ? TVec2(0, 0) : TVec2{ X / value, Y / value }; }
		...
		TVec2 operator-() const& { return TVec2(*this) * static_cast<T>(-1); }
	...
	template <class T>
	const TVec2<T> TVec2<T>::AXIS_X = TVec2<T>(1, 0);

	template <class T>
	const TVec2<T> TVec2<T>::AXIS_Y = TVec2<T>(0, 1);
