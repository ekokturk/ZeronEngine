...
namespace Zeron
{
	...
	class TVec3 {
	  ...
	  public:
		...
		bool operator==(const TVec3& other) const { return X == other.X && Y == other.Y && Z == other.Z; }
		...
		TVec3 operator+(T value) const { return { X + value, Y + value, Z + value }; }

		TVec3 operator-(T value) const { return { X - value, Y - value, Z - value }; }

		TVec3 operator*(T value) const { return { X * value, Y * value, Z * value }; }

		TVec3 operator/(T value) const { return value == 0 ? TVec3(0, 0, 0) : TVec3{ X / value, Y / value, Z / value }; }
		...
		TVec3 operator-() const& { return TVec3(*this) * static_cast<T>(-1); }
	...
	template <class T>
	const TVec3<T> TVec3<T>::AXIS_X = TVec3<T>(1, 0, 0);

	template <class T>
	const TVec3<T> TVec3<T>::AXIS_Y = TVec3<T>(0, 1, 0);

	template <class T>
	const TVec3<T> TVec3<T>::AXIS_Z = TVec3<T>(0, 0, 1);
